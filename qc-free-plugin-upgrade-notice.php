...
if( !class_exists('QcPluginUpgradeToProNotice') )
{
	class QcPluginUpgradeToProNotice {
		...
		public $upgrade_link = "https://www.dna88.com/product/highlight/";
		...
		function func_show_upgrade_link_with_action_links( $links )
		{
			...
			// ) );
			// $links = array_merge( $links, array(
			// 	'<a  href="' . esc_url('https://www.quantumcloud.com/resources/free-support/' ) . '" target="">' . __( 'Support', 'quantumcloud' ) . '</a>'
			// ) );
			$links = array_merge( $links, array(
...
$instance_highlite = new QcPluginUpgradeToProNotice();

if( is_admin() )
{ 
	
	//Uncommnent and Set these instance variables as per the requirements
	$instance_highlite->upgrade_link = "https://www.dna88.com/product/highlight/";


	$instance_highlite->plugin_slug = 'highlight'; //Plugin Slug. i.e. Folder Name
	$instance_highlite->plugin_main_file = 'qc-project-highlight-main.php'; //Primary file of the pluign
	$instance_highlite->plugin_menu_slug = 'qcld-highlight'; //Main Menu Slug

	$instance_highlite->show_with_action_links = true; //show in plugin.php page
	$instance_highlite->show_with_meta_links = true; //show in plugin.php page
	$instance_highlite->show_with_plugin_menu = true; //show inside parent menu of the plugin

	if( $instance_highlite->check_if_plugin_page() )
	{
		$instance_highlite->hook_with_plugin_action_links();
		$instance_highlite->hook_with_plugin_meta_links();
	}
	
	$instance_highlite->hook_with_plugin_submenu();
